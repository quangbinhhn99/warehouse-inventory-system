...
if (isset($_POST['add_request'])) {
  ...
  if (empty($errors)) {
    ...
    $query .= "sku, idPro, number_datHang, number, number_finished,number_phanBo, total_vai, total_chi, total_cuc ,dateStart,dateEnd, status, note";
    $query .= ") VALUES (";
    $query .= "'{$sku}', '{$idPro}', '{$number}','{$number}', '{$number_finished}' ,'0', '{$total_vai}' , '{$total_chi}', '{$total_cuc}', '{$dateStart}', '{$dateEnd}', '{$status}', '{$note}'";
    $query .= ")";
